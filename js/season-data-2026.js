...
const FESTIVALS_2026 = [
  ...
  },
  {
    ...
    races:null,
    headline:'Day 4 priced — the final day. 8/8 gated, 18 picks, £18.00, 109 runners. The whole-field scan found the festival\'s ONLY first-time wind operation — Mo Of Cairo, w1, 14/1, 66 days off and steaming — and it is backed as the LONG, which is the exact marker Step 3 item 11 was written into the engine for after it was missed on Day 1. The LONG opens twice and both races declared EXACTLY sixteen runners: zero place-band margin, so one non-runner shuts either slot. The 15:05 has twenty runners and the slot is held back against the gate\'s own output, because a Group 3 is not a five-place race — the same call Day 1 proved right when a 17-runner non-handicap paid three. Eight first-time headgear markers on 109 runners, four of them on the first two in the betting. A new kind of signal appeared on Sunday morning: Spotlight has rated Forty Years On THREE STARS, the first multi-star row in the archive — every other tip this meeting is a single star. It is a confidence rating rather than a crossover, it lands on our NB, and the slots do not move; printed at full strength so the question of whether a column\'s own confidence deserves weight can be judged on the result. Zero shorter prices anywhere this morning against three on Saturday. Day 3 priced with the card\'s first 3x (Enceladus, landing on a pick rule 8 had already demoted to NB). Day 2 settled +£3.47 (+20.4%), Day 1 −£10.60 (−66.2%).',
